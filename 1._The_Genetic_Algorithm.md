...
Player A's strategy is represented with a vector of numerical values indicating the amount to bet for each card (s)he could receive. If we consider a game where the bets 0, 2, 4, 6, 8, 10 are allowed and the cards are 1, 2, 3, 4, the following is a possible strategy for player A

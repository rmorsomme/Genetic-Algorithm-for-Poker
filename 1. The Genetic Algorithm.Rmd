...
# Set of parent strategies (relatively good strategies)
print(parents)
```
...
generate_A <- function(fitness, pop, n_parents, dim_pop_A, dimname_pop_A,
  ...
  # Crossover
  pop <- array(
    data = sample(name_parents, size = prod(dim_pop_A), replace = T,
                  prob = exp(fitness_parents)),
    dim = dim_pop_A, 
    dimnames = dimname_pop_A
    )
  ...
  # Mutation
  mutation_location <- array(
    data = sample(c(T, F), size = prod(dim_pop_A), TRUE,
                  prob = c(mutation_rate, 1-mutation_rate)),
    dim = dim_pop_A,
    dimnames = dimname_pop_A
    )
...
generate_B <- function(fitness, pop, n_parents, dim_pop_B, dimname_pop_B,
  ...
  mutation_location <- array(
    data = sample(c(T, F), size = prod(dim_pop_B), replace = T,
                  prob = c(mutation_rate, 1-mutation_rate)),
    dim = dim_pop_B,
    dimnames = dimname_pop_B
    )
